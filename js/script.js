(function(window) {
    ...
    window.onscroll = function() {
        ...
        if(scrollTop >=150){
            myTool.$('top_nav').style.display = "block";
            myTool.$('elevator').style.display = "block";
        }else{
            myTool.$('top_nav').style.display = "none";
            myTool.$('elevator').style.display = "none";
        }
        
        // 4.4. 滚回顶部
        $('elevator').onclick = function () {
            myTool.buffer(document.documentElement, {scrollTop: 0}, null);
        }
    }
    
    //  5.监听点击按钮
    myTool.$("login").onclick = function () {
       $("mask").style.display = "block";
    };
    myTool.$("close_btn").onclick = function () {
...
function bannerAutoPlay() {
   // 1. 获取所有的li标签
   var lis = myTool.$("slider_banner").getElementsByTagName("li");
   var index = 0;
   // 2. 开始定时器
   setInterval(function () {
       ...
       for(var i=0; i<lis.length; i++){
           var singerLi = lis[i];
           myTool.buffer(singerLi, {opacity: 0}, null);
       }
       myTool.buffer(lis[index], {opacity: 1}, null);
...
function autoCreateImg(){
    ...
    for (var i = 0; i < 30; i++) {
        ...
        // 1.2.3 拼接
        str += htmlStr;
        myTool.$("dom_pull").innerHTML = str;
        
        // 1.2.4绑定事件
        var wrapBox = myTool.$("dom_pull").getElementsByClassName("box");
        var wrappic = myTool.$("dom_pull").getElementsByClassName("pic");
...
function tab() {
	// 1. 获取需要的标签
	var allLis = myTool.$('tab_header').getElementsByTagName('li');
	var doms = myTool.$('tab_content').getElementsByClassName('dom');
	var lastOne = 0;
